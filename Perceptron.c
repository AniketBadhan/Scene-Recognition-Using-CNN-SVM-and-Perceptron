...
void PerceptronWeightCalculation(int output_number, float temp_weight_row[491], float train_data[500][490], int train_label[500], float lambda);
int PerceptronTesting(float WeightMatrix[OUTPUTCLASS][491], int start_number, int end_number);
...
int main(int argc, char *argv[])
{
	...
	fclose(fp4);
	//For calculating number of elements for which Perceptron will find the weight matrix
	...
	for (output_number = donearray[myid]; output_number< donearray[myid]+sizearray[myid]; output_number++)
	{
		...
		PerceptronWeightCalculation(outputarray[output_number], temp_weight_row, train_data, train_label, lambda);
	...
	local_accuracy = PerceptronTesting(WeightMatrix, start_number, end_number);
...
/*Perceptron Training, Calculate weight matrix*/
void PerceptronWeightCalculation(int output_number, float temp_weight_row[491], float train_data[500][490], int train_label[500], float lambda)
{
	int i,j,k,l;
	...
	for(i=0; i<500; i++){
		...
		if(sum > 0)
			...
			predictedoutput = -1;
		if (predictedoutput != temp_train_label[i])
			for(k = 0; k < WEIGHTSIZE; k++){
				if(k < WEIGHTSIZE-1){
					temp_weight_row[k] = temp_weight_row[k] + (temp_train_label[i]*train_data[i][k]);
				}
				else{
					temp_weight_row[k] = temp_weight_row[k] + (temp_train_label[i]);
				}
			}
		}
}

//Perceptron Testing, calculating the local accuracy
int PerceptronTesting(float WeightMatrix[3][491], int start_number, int end_number)
{
	int i,j,k,l;
